...
const DasAllMeals = () => {
  ...
  const {
    ...
    queryKey: ["dasAllMeals"],
    queryFn: async () => {
      const { data } = await axiosSecure.get('/dasAllMeals');
      return data;
    },
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const { data } = await axiosSecure.delete(`/meal/${id}`);
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] border-2 border-cyan-300 space-y-7">
      <div className="m-2">
        <TableHeaderText text={"My All Reviews"} count={allMeals.length} />
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 ">
                      {allMeals.map((review) => (
                        <tr key={review?._id}>
                          ...
                          {/* View meal btn */}
                          <td className="px-4 py-4 text-sm whitespace-nowrap">
                            <Link to={`/mealDetails/${review?._id}`}>
                              <div className="flex justify-center  items-center">
